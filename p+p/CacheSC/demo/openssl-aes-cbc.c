...
int main(int argc, char **argv) {
    ...
    if (argc == 2)
        sample_cnt = atoi(argv[1]);//把字符串转换成整型数的一个函数
    if (sample_cnt < 0)
        usage(argv[0]);
    ...
    assert(res);
    memset(res, 0, res_size);//将某一块内存中的内容全部设置为指定的值,0
    ...
    // which is added to be able to choose any cache set offset we want.初始化明文、密文和密钥的数组并放置它们
    //在内存中尽量避免碰撞。
    //将ctx、ct、pt和key放在不同的缓存集中（如果可能）。
    //`sets*CACHELINE_SIZE`是覆盖所有缓存集的大小它可以选择我们想要的任何缓存集偏移量
    ...
    /*unsigned char *ct   = data + CACHELINE_SIZE * ((ctx_cache_set
                          + get_spanned_cache_lines(cache_ctx, sizeof(EVP_CIPHER_CTX))) //error
                          % cache_ctx->sets);*/
    unsigned char *ct   = data + CACHELINE_SIZE * ((ctx_cache_set
                          + get_spanned_cache_lines(cache_ctx, sizeof(aes_ctx))) //error
                          % cache_ctx->sets);
    ...
    //预测访问集取决于Te0偏移量。
    //这可以通过监视缓存使用情况来检测。或者，一个
    //还可以修补openssl库以导出此地址，例如使用
    //函数调用EVP_aes_get_Te0_addr（），然后按如下方式查找集合：
    //获取缓存集（cache_ctx，（void*）（EVP_aes_get_Te0_addr（）
    //+key_arr[目标字节]*4））
    ...
    for (i = 0; i < sample_cnt; ++i) {
        memcpy(pt, pt_arr + i * PT_LEN, PT_LEN);//函数的功能是从源内存地址的起始位置开始拷贝若干个字节到目标内存地址中，即从源source中拷贝n个字节到目标destin中。
        memcpy(key, key_arr + i * KEY_LEN, KEY_LEN);
