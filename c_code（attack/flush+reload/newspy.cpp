...
int main()
{
  ...
  size_t recordtime[40000];
  unsigned char recordplain[40000][16];
  unsigned char recordcipher[40000][16];

for(int roundsum=0;roundsum<1;roundsum++)
{
  for (int i=0; i<16; i++) {
    for (int j=0; j<256; j++) {
      totalEncs[i][j] = 0;
  ...
  sum = 0;
  char* probe[] = { 
    base + 0x1db000, base + 0x1db400, base + 0x1db800, base + 0x1dbc00
  };
  ...
  //save data
  char name[20];
  sprintf(name,"data-%d.txt",key[14]);
  FILE *fpwrite=fopen(name,"w");
  if(fpwrite==NULL) printf("error");
  for(int i=0;i<NUMBER_OF_ENCRYPTIONS; i++)
  {
    fprintf(fpwrite,"%lu ",recordtime[i]);
  }
  fclose(fpwrite);
  printf("save time data done\n");
  
  sprintf(name,"plain-%d.txt",key[14]);
  ...
  for(int i=0;i<NUMBER_OF_ENCRYPTIONS; i++)
    {
      for(int k=0;k<16;k++)
      {
    	fprintf(fp,"%x",recordplain[i][k]);
      }
     fprintf(fp,"\t");
    }
  fclose(fp);
  printf("save plaintext data done\n");
   
  sprintf(name,"cipher-%d.txt",key[14]);
  FILE *p=fopen(name,"w");
  ...
  for(int i=0;i<NUMBER_OF_ENCRYPTIONS; i++)
    {
      for(int k=0;k<16;k++)
      {
    	fprintf(p,"%u ",recordcipher[i][k]);
      }
      fprintf(p,"\t");
    }
  fclose(p);
  printf("save cipher data done\n");
  ...
  key[14]+=1;// key count+1 try to attack key+1
